...
impl Encoder {
    ...
    fn embedding_var(&self, x: Tensor<2>, std: f64) -> Tensor<2> {
        let [n, d] = x.dims();
...
fn main() {
    ...
    let make_split = |per_class: usize, seed: u64| -> (Tensor<2>, Vec<i64>) {
        let n = N_CLASS * per_class;
        dev.seed(seed);
        let mut xv = Tensor::<2>::random([n, D_IN], Distribution::Normal(0.0, 0.7), &dev)
            .to_data()
            .try_to_vec::<f32>()
            .unwrap();
        ...
        (
            Tensor::<2>::from_data(TensorData::new(xv, [n, D_IN]), &dev),
            labels,
        )
    };
    ...
    let same: Vec<i64> = (0..N_TRAIN)
        ...
        .collect();
    let perm_t = Tensor::<1, Int>::from_data(
        TensorData::new(
    ...
    let same_t = Tensor::<1, Int>::from_data(TensorData::new(same, [N_TRAIN]), &dev);

    dev.seed(0x7A91_1000);
    let init = Encoder::init(&dev);
    let train = |mut model: Encoder, lambda: f64| -> Encoder {
    ...
    let centroids = |model: &Encoder| -> Vec<f32> {
        let e = model
    ...
    dev.seed(0x7A91_2000);
    let draws = 10;
    let noisy_test_draws: Vec<Tensor<2>> = (0..draws)
        .map(|_| Tensor::<2>::random([N_TEST, D_IN], Distribution::Normal(0.0, TEST_STD), &dev))
        .collect();
    let accuracy = |model: &Encoder, cen: &[f32], noise_draws: &[Tensor<2>]| -> f64 {
        let mut correct = 0;
    ...
    let clean_draw = vec![Tensor::<2>::zeros([N_TEST, D_IN], &dev)];
    ...
    let propagated_variance = |model: &Encoder| -> f64 {
        model
            .embedding_var(x_train.clone(), TRAIN_STD)
            .mean()
            .into_data()
    ...
    let embedding_rms = |model: &Encoder| -> f64 {
        let embedding = model
...
mod tests {
    ...
    fn cloned_baseline_has_identical_forward_predictions() {
        ...
        device.seed(0x7A91_7E57);
        let baseline = Encoder::init(&device);
